...
// Exam catalog
const exams = [
    { examId: OLD_ID, title: 'English Grammar Exam', description: 'Comprehensive exam covering grammar fundamentals: quantifiers, prepositions, modals, and more.', totalQuestions: 30, active: true, order: 1 },
    { examId: NEW_ID, title: 'Simple Present and Present Continuous', description: 'Advanced exam on stative vs. dynamic verbs, scheduled events, and temporary vs. permanent actions.', totalQuestions: 30, active: true, order: 2 }
];

// 30 new questions (compact)
const qs = [
    { id:1, instruction:"Choose the correct verb forms to complete the sentence:", englishText:"The official delegation ________ at 10:00 AM tomorrow, so we ________ for the airport in an hour.", options:["arrives / are leaving","is arriving / leave","arrives / leave","is arriving / are leaving"], correctIndex:0, hint:"Match the simple tense to the fixed timetable and the continuous tense to the personal arrangement.", explanation:"The simple present ('arrives') is used for fixed official schedules, while the present continuous ('are leaving') is used for personal future arrangements." },
...
async function main() {
    console.log("=== MIGRATION via artifacts collection ===\n");
    await signInWithEmailAndPassword(auth, "lucimar132803@gmail.com", "luci2026");
    console.log("Authenticated.\n");

    // Write exam catalog entries
    for (const exam of exams) {
        await setDoc(doc(db, 'artifacts', `exam--${exam.examId}`), { _type: 'exam', ...exam, createdAt: new Date().toISOString() });
        console.log(`Exam: ${exam.title}`);
    }

    // Write new questions
    for (const q of qs) {
        const docId = `q--sppc-${q.id.toString().padStart(2, '0')}`;
        await setDoc(doc(db, 'artifacts', docId), { _type: 'question', examId: NEW_ID, ...q });
    }
